...
let%test_unit "hits target after entry" =
  ...
  let exec = EP.default ~tick_size:0.25 in
  ...
  let state1, trades1 =
    TT.step ~exec ~plan ~state:pending ~bar:bar_entry ~record_trade:(make_trade ~plan)
  in
  ...
  let _, trades2 =
    TT.step ~exec ~plan ~state:state1 ~bar:bar_target ~record_trade:(make_trade ~plan)
  in
...
let%test_unit "moves to break_even then stops at entry" =
  ...
  let state2, _ = TT.step ~exec ~plan ~state:state1 ~bar:bar_be ~record_trade:(make_trade ~plan) in
  let bar_stop = mk_bar ~minute:3 ~high:100.8 ~low:99.8 ~close:100.0 in
  let _, trades = TT.step ~exec ~plan ~state:state2 ~bar:bar_stop ~record_trade:(make_trade ~plan) in
...
let%test_unit "cancelled when price breaches cancel before entry" =
  ...
  let state, trades =
    TT.step ~exec ~plan ~state:(TT.init_pending ~qty:1.0 ~latency_bars:exec.latency_bars ~cancel_after:exec.cancel_latency_bars)
      ~bar:bar_cancel ~record_trade:(make_trade ~plan)
  in
...
let%test_unit "stop takes precedence over target in same bar" =
  ...
  let _, trades = TT.step ~exec ~plan ~state:state1 ~bar ~record_trade:(make_trade ~plan) in
  assert (List.length trades = 1);
  let t = List.hd_exn trades in
  assert (Poly.(t.exit_reason = Types.Stop))
