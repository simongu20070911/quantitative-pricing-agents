...
let make ?bounds ?scale ?fixed ?integer ?tunable ?domain ?description ~name ~default () =
  ...
  let tunable = Option.value tunable ~default:true in
  let domain =
    match domain with
    | Some d -> d
    | None -> if integer then Integer else Continuous
  in
  let integer =
    match domain with
    | Integer | Discrete _ | Categorical _ -> true
    | Continuous -> integer
  in
  let description = Option.value description ~default:"" in
  { name; default; bounds; scale; fixed; integer; tunable; domain; description }
