...
type config = {
  tick_size : float;
  tick_value : float;
  fee_per_contract : float;        (** fee remains in USD, applied per contract *)
...
let apply ~(qty : float) (cfg : config) (t : trade) : trade =
  ...
  (* Slippage modeled inside Execution_model; cost-level slippage removed. *)
  let cost_fees_usd = cfg.fee_per_contract *. qty in
  let net_usd = gross_usd -. cost_fees_usd in
