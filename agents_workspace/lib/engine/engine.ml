...
let run (strategy : strategy) ~(filename : string) : run_result =
  ...
  let finalize_day (s : S.t) =
    ...
    | Some st ->
        ...
        let acc = Pnl.add_trades s.acc trades in
  ...
  let init_state : S.t = { current_date = None; policy_state = None; last_session_bar = None; acc = Pnl.empty } in
  ...
  let daily_pnl, daily_pnl_usd, daily_pnl_pct = Pnl.to_alists_unsorted acc in
