...
class LSTMAttnReturns(nn.Module):
    ...
    def forward(self, x):
        ...
        scores = self.attn_fc(out).squeeze(-1)  # (B, T)
        weights = torch.softmax(scores, dim=1)  # (B, T)
        ctx = torch.sum(weights.unsqueeze(-1) * out, dim=1)  # (B, H)
        logits = self.out_fc(ctx).squeeze(-1)  # (B,)
        return logits
...
def main():
    ...
    # Train <=2018, test >=2019 (same split as earlier LSTM)
    ...
    model = LSTMAttnReturns(input_dim=1, hidden_dim=32, num_layers=1).to(device)
    ...
    print(f"LSTM+Attn(30 returns) test AUC: {auc:.6f}")

    # Sign-flip backtest with cost 0.07 + 0.25
    COST = 0.07 + 0.25
    ...
    print("LSTM+Attn signflip trades_used", len(trades_pnl))
    if len(trades_pnl) == 0:
        return
    mean_trade = float(trades_pnl.mean())
    std_trade = float(trades_pnl.std())
    print(
        "LSTM+Attn signflip per-trade mean_pts",
        mean_trade,
        "std_pts",
        std_trade,
    )
    ...
    print(
        "LSTM+Attn signflip days_with_trades",
        len(by_day),
        "mean_daily_pts",
        mean_daily,
        "std_daily_pts",
    ...
    print(
        "LSTM+Attn signflip daily_sharpe",
        sharpe_daily,
        "annualized_sharpe",
        sharpe_annual,
    )
