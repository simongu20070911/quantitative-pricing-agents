...
def build_returns(df: pd.DataFrame) -> np.ndarray:
    ...
    for i in range(1, len(close_np)):
        if close_np[i - 1] > 0 and close_np[i] > 0:
            ...
            ret[i] = 0.0
    # Standardize within segment (train/test separately)
    mu = ret.mean()
    sigma = ret.std()
    if sigma > 0:
        ret = (ret - mu) / sigma
...
def main():
    ...
    seq_len = 120  # 120 x 1-minute bars
    ...
    print(f"LSTM(120 x 1m returns) test AUC: {auc:.6f}")

    # Always-in sign-flip backtest with ES-like cost
    COST = 0.07 + 0.25  # 1 tick + fees
    ...
    print("LSTM-120 signflip trades_used", len(trades_pnl))
    if len(trades_pnl) == 0:
        return
    mean_trade = float(trades_pnl.mean())
    std_trade = float(trades_pnl.std())
    print(
        "LSTM-120 signflip per-trade mean_pts",
        mean_trade,
        "std_pts",
        std_trade,
    )
    ...
    print(
        "LSTM-120 signflip days_with_trades",
        len(by_day),
        "mean_daily_pts",
        mean_daily,
        "std_daily_pts",
    ...
    print(
        "LSTM-120 signflip daily_sharpe",
        sharpe_daily,
        "annualized_sharpe",
        sharpe_annual,
    )
