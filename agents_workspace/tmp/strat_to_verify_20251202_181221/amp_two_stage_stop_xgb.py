...
def build_stop_labels(
    ...
    For the 3m export `ml_export_es_3m_amp_50_20_full.csv`, each row at time t
    represents the 3-minute bar covering [t, t+2]. We therefore shift the stop
    entry to the 1m bar at t+2 so that features and entry price are aligned
    at the 3m bar close.
    """
    return compute_stop_labels(df, raw, tick=tick, horizon=horizon, entry_shift_min=2)
...
def main():
    ...
    # Align to 3m bar close: the export row at time t covers [t, t+2],
    # so we map to the underlying 1m bar at t+2 (same as in build_stop_labels).
    ENTRY_SHIFT_MIN = 2
