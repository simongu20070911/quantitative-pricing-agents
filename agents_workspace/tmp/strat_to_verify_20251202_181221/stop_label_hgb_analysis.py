...
def compute_stop_labels(
    ...
    For each bar t in df:
      - Map to raw index i via date+time, optionally shifted forward by
        `entry_shift_min` minutes. For 3m exports produced by ml_export_main,
        the row at time t represents [t, t+2], so using entry_shift_min=2
        aligns the stop entry with the 3m bar close at t+2.
      - Stop at close_{t+shift} + tick.
    ...
    for _, row in df.iterrows():
        # Optionally shift the entry minute to align with a higher-timeframe
        # bar close (e.g. 3m bar covering [t, t+2] uses close at t+2).
