...
def build_returns(df: pd.DataFrame) -> np.ndarray:
    ...
    for i in range(1, len(close_np)):
        if close_np[i - 1] > 0 and close_np[i] > 0:
            ...
            ret[i] = 0.0
    # Standardize within segment (train/test separately to avoid leakage)
    mu = ret.mean()
    sigma = ret.std()
    if sigma > 0:
        ret = (ret - mu) / sigma
...
def main():
    ...
    seq_len = 120  # 120 x 1-minute bars (~2 hours)
    ...
    print(f"Attention(120 x 1m returns) test AUC: {auc:.6f}")

    # Optional: simple sign-flip always-in backtest (no cost here by default)
    ...
    COST = 0.0  # set >0 if you want to include costs
    n = len(test_df_valid)
    state = 0
    entry_px = None
    trades_pnl = []
    ...
    print("Attention-120 signflip trades_used", len(trades_pnl))
    if len(trades_pnl) > 0:
        mean_trade = float(trades_pnl.mean())
        std_trade = float(trades_pnl.std())
        print(
            "Attention-120 signflip per-trade mean_pts",
            mean_trade,
            "std_pts",
            std_trade,
        )
        # Daily Sharpe (no cost by default)
        ...
        print(
            "Attention-120 signflip days_with_trades",
            len(by_day),
            "mean_daily_pts",
            mean_daily,
            "std_daily_pts",
        ...
        print(
            "Attention-120 signflip daily_sharpe",
            sharpe_daily,
            "annualized_sharpe",
            sharpe_annual,
        )
